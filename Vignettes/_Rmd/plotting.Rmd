...
cite.pkg <- function(p, entries) {
  paste0("[@", paste(make.cite.keys(p, entries), collapse = ";"), "]")
}
...
make.bib(c("hyperSpec", "lattice", "latticeExtra", "rgl", "ggplot2",
  "playwith", "plotrix",  "deldir", "tripack"),
...
```{r FIXME-1, include=FALSE}
# FIXME: Is "latticist" really mentioned in this vignette?
```

In addition, package **tripack**, and package **latticist** are mentioned, but not used in this vignette.
...
**`levelplot()`{.r}** 
  ~  package **hyperSpec** has a method for package **lattice**`r cite.pkg("lattice")` function `levelplot()`{.r}
...
# Customized Plots of Spectra: `plotspc()`{.r}
<!-- \mFun{`plotspc()`{.r}} -->
...
## Plot Severel Wavelength Ranges
...
In the example, standard deviation is used.  "
```
...
```{r FIXME-2, include=FALSE}
# FIXME: gramatical structure of these sentences are dificult to understand.
# Does this alternative formulation reflect the idea of the sub-chapter well enough?
...
Alternatively, the name of extra data column can be used for grouping (Fig. \@ref(fig:stacked2)).
...
## Calibration Plots: `plotc()`{.r} {#sec:line-calibr-plots}
...
# Levelplot: `levelplot()`{.r} {#sec:levelplot}
...
# False-Colour Maps: `plotmap()`{.r} {#false-colour-maps}
...
## Plotting Maps with Manully Specified X and Y Axes {#plotmap-manual-xy}
...
```{r include=FALSE}
CAPTION = "False colour map with explicit spacification of x and y axes.  "
```
...
Factor variables may also be used for colour coding (Fig. \@ref(fig:plotmap-clu)).
...
The plotting of color maps is done via R package **lattice** (aka Trellis graphic approach), which is highly customizable. 
...
in the [R color cheatsheet](https://www.nceas.ucsb.edu/~frazier/RSpatialGuides/colorPaletteCheatsheet.pdf).
...
Fig. \@ref(fig:plotmap-pca2) demonstrates an exaple to plot the maps of principal componens scores.
...
Fig. \@ref(fig:voronoi-2) shows an example of Voronoi plot.
...
One possibility to obtain a correct map is using `plotvoronoi()`{.r} instead which will construct a mosaic-like image with the respective"ixe"areas being centered around the actually recorded `$x` and `$y` position (Fig. \@ref(fig:uneven-II)).
...
knitr::include_graphics("fig-3D-01.png")
```
...
# Using **_ggplot2_** with **_hyperSpec_** objects {#sec:ggplot2}

```{r FIXME-3, include=FALSE}
# FIXME: ggplot2 method for "hyperSpec" objects can be easily created
```

Class `hyperSpec`{.r} objects do not yet directly support plotting with package **ggplot2**`r cite.pkg("ggplot2")`.
...
Fig. \@ref(fig:ggplotmap) shows map with package **ggplot2**.
...
```{r FIXME-4, include=FALSE}
# FIXME: in my opinion, "vignettes.defs" shouldn't be mentioned
```
