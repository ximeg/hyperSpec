...
package **baseline**`r cite.pkg("baseline")`, exists that provides many more methods to fit
...
- **`npts.min()`{.r}:** minimal number of data points per spectrum to be used for the fit.
  ...
  If `npts.min()`{.r} $\leq$ {poly.order}, a warning is issued and \Rcode{npts.min <- poly.order
    + 1} is used.
...
Besides defining a minimal number of supporting points, a"oise leve"may be given.  Consider a
