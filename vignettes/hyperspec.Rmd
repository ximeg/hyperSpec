...
link-citations: yes
bibliography: man/hyperSpec-into-pkg.bib
biblio-style: plain
...
#   fig.path   = "man/figures/hyperSpec-into--"
# )
```
...
make_bib(
  ...
  file = "man/hyperSpec-into-pkg.bib"
)
```
...
`bug.report (package = "hyperSpec")`{.r}
will take you to package **hyperSpec**'s issue tracking page at
...
package **hyperSpec** is a R package that allows convenient handling of \index{hyperspectral data sets}hyperspectral
data sets, i.e., data sets combining spectra with further data on a per-spectrum basis. The spectra can
be anything that is recorded over a common discretized axis.

This vignette gives an introduction on basic working techniques using the R package package **hyperSpec**.  This is
done mostly from a spectroscopic point of view: rather than going through the functions provided by
package **hyperSpec**, it is organized in spectroscopic tasks. However, the functions discussed are printed on the
margin for a quick overview.

package **hyperSpec** comes with five data sets,
<!-- wavelength:  -->
-  [`chondro`{.r}] \index{data sets!chondro}a Raman map of chondrocytes in cartilage,
-  [`flu`{.r}] \index{data sets!flu}a set of fluorescence spectra of a calibration series, and
-  [`laser`{.r}] \index{data sets!laser}a time series of an unstable laser emission
-  [`paracetamol`{.r}] \index{data sets!paracetamol}a Raman spectrum of paracetamol
  (acetaminophene) ranging from 100 to $cm^{-1}${3200} with overlapping wavelength ranges.
-  [`barbiturates`{.r}] \index{data sets!barbiturates}GC-MS spectra with differing wavelength
  axes as a list of `r length (barbiturates)` `hyperSpec`{.r} objects.
...
R's help system using `?~command`{.r}.
...
<!-- wavelength:  -->
-  [wavelength:] \index{wavelength}spectral abscissa

  frequency, wavenumbers, chemical shift, Raman shift,
  $\frac{m}{z}$, etc.
-  [intensity:] \index{intensity}spectral ordinate

  transmission, absorbance, $\frac{e^{-}}{s}$, intensity, \textellipsis
-  [extra data:] \index{extra data}further information/data belonging to each spectrum
  ...
  `hyperSpec`{.r} object may contain arbitrary numbers of extra data columns.

In R, slots of a S4 class are accessed by the \index{`"@`{.r} operator}`@`{.r} operator. In
this vignette, the notation `@xxx`{.r} will thus mean \emph"lot xxx of an objec".  Likewise,
named elements of a `list`{.r} and columns of a `data.frame`{.r} are accessed by the \index{`\$`{.r}
  operator}`\$`{.r} operator, and `\$xxx`{.r} will be used for \emph"olumn xx", and as an
abbreviation for \emph"olumn xxx of the `data.frame`{.r} in slot data of the objec" (the structure of `hyperSpec`{.r}
...
`bug.report()`{.r} will either open an email to the package maintainer or
the issue tracker URL given in the package `DESCRIPTION`.

```{r eval=FALSE}
bug.report (package = "hyperSpec")
```
will take you to package **hyperSpec**'s issue tracking page at
<https://github.com/cbeleites/hyperSpec/issues>. It also displays essential
information about your installation which can help in tracking down the bug.

We're always happy about contributions and tag issues that may be tackled immediately by"elp wante" Please note that I (Claudia, the official maintainer) may be rather slow in answering pull requests: at the moment I'm travelig a lot professionally so it may take several weeks until I can find some calm chunk of time to do more for hyperSpec than emergency fixes. However, this does not mean that I won't do so: I can tell quickly if a pull request won't fit at all into package **hyperSpec**.

## Generic Functions

*\index{Generic Functions*Generic Functions} are functions that apply to a wide range of data
types or classes, e.g., `plot()`{.r}, `print()`{.r}, mathematical operators, etc. These functions
can be implemented in a specialized way by each class.  `hyperSpec`{.r} implements with a variety
...
his *workspace* at any time. This is also true for methods belonging to a certain class.
Neither restart of R nor reloading of the package or anything the like is needed. If the original
function resides in a namespace (as it is the case for all functions in package **hyperSpec**), the original function
is not deleted. It is just masked by the user's new function but stays accessible via the `::`{.r}
operator.

The same is true for"orma"variables: You may create changed copies of the example data sets,
work with these and then"ese"to package **hyperSpec**'s version of the data set by removing the object in your
workspace.

This offers the opportunity of easily writing specialized functions that are adapted to specific
tasks. package **hyperSpec**'s vignettes use this to set up special versions of the lattice graphics functions that
are already wrapped in `print()`{.r} (see also
[R
  FAQ: Why do lattice/trellis graphics not work?](http://cran.r-project.org/doc/FAQ/R-FAQ.html#Why-do-lattice_002ftrellis-graphics-not-work_003f)) and allow the code in the code chunks of the
vignettes to be exactly what one would type during an interactive \R session. For the code, check the
`vignettes.defs` file accompanying all package **hyperSpec** vignettes.

## Validity Checking
S4 classes have a mechanism to define and enforce that the data actually stored in the object is
appropriate for this class. In other words, there is a mechanism of *\index{validity checking*validity checking}.

The functions provided by package **hyperSpec** check the validity of `hyperSpec`{.r} objects at the beginning, and
...
non-generic functions should first ensure that the argument actually is a `hyperSpec`{.r} object. The two tasks
are accomplished by:
...
\index{operators}Operators such as \verb/+/, `-`,`*`, `%%`, etc. are in fact functions
in R. Thus they
can be handed over as arguments to other functions (particularly to the vectorization functions
`*apply()`{.r}, `sweep()`{.r}, etc.). In this case the name of the function must be quoted:
``-`` is the recommended style (although `"-"` will often work as well), e.g.,:
...
```
The actual name of the function is `wl<-` and must be quoted in order to avoid confusion with
an assignment to variable `wl`: ``wl<-``.

# Loading and the package and configuration {#sec:loading-package}
...
\index{options!gc}
The global behaviour of package **hyperSpec** can be configured via options. The values of the options are
retrieved with `hy.getOptions()`{.r} and `hy.getOption()`{.r}, and changed with
`hy.setOptions()`{.r}. Table~\@ref(tab:options) gives an overview.
\begin{table}[tb]
  \begin{small}
  \begin{tabular}{p{.1\textwidth}p{.15\textwidth}p{.45\textwidth}p{.2\textwidth}}
    ...
    **name** & **default value** (range) & **description**                       & **used by**                                        
    ...
    debuglevel    & 0 (1L 2L)                      & amount of debugging information produced   & `spc.identify()`{.r}, `map.identify()`{.r}, `spc.rubberband()`{.r}, various file import functions      

    gc            & FALSE                          & triggers frequent calling of `gc ()`{.r} & `read.ENVI()`{.r},\newline`new ("hyperSpec")`{.r} 
    ...
    wl.tolerance & $\sqrt{`.Machine\$.double.eps`{.r}}$ & tolerance for comparisons of the wavelength axis & `rbind()`{.r}, `rbind2()`{.r}, `bind()`{.r}` ("r", ...)`{.r}, `all.equal()`{.r}, `collapse()`{.r}

    file.remove.emptyspc & TRUE & automatic removing of empty spectra & file import functions, see `vignette ("fileio")`{.r}

    file.keep.name & TRUE & automatic recording of file name in column `\$filename`{.r} & file import functions, see `vignette ("fileio")`{.r}

    plot.spc.nmax        & 25                                & number of spectra to be plotted by default       & `plotspc()`{.r}

 ggplot.spc.nmax      & 10                                &                                                  & `qplotspc()`{.r}

    \hline
    %
  \end{tabular}
  \end{small}
  \caption{package **hyperSpec** options. Please refer to the documentation of the respective functions for details about the effect of the options.}
   {#tab:options}
\end{table}
...
`hyperSpec`{.r} is a S4 (or new-style) class. Four slots contain the parts of the object:
<!-- @wavelength  -->
...
While the parts of the `hyperSpec`{.r} object can be accessed directly, it is good practice to use the functions
provided by package **hyperSpec** to handle the objects rather than accessing the slots directly
(tab.~\@ref(tab:getset)). This also ensures that proper (*valid*) objects are returned. In some
...
\begin{table}[t]
 ...
 %] emacs font lock gets confused otherwise
    @label        & `labels`                                                                  & `labels<-`                     

    \bottomrule
  \end{tabular}
  \caption{Get and set functions for the slots of `hyperSpec`{.r} objects}
   {#tab:getset}
\end{table}

Most of the data is stored in `@data`{.r}. This `data.frame`{.r} has one special column, `\$spc`{.r}. It is
the column that actually contains the spectra. The spectra are stored in a matrix inside this column,
as illustrated in figure~\@ref(fig:structure). Even if there are no spectra, `\$spc`{.r} must still
be present. It is then a matrix with zero columns.
\begin{figure}[bt]
\noindent \centering
\includegraphics{strukturhyperspec}
\caption{ {#fig:structure}The structure of the data in a package **hyperSpec** object.}
\end{figure}

Slot `@label`{.r} contains an element for each of the columns in `@data`{.r} plus one holding the
label for the wavelength axis, `.wavelength`{.r}. They are accessed by their names which must be
the same for columns of `@data`{.r} and the list elements. The elements of the list may be anything
suitable for axis annotations, i.e., they should be either character strings or expressions for"rett"axis annotations (see e.g., figure~\@ref(fig-spcloess) on page~\pageref{fig-spcloess}).  To
get familiar with expressions for axis annotation, see `? plotmath` and `demo (plotmath)`.

# Functions provided by package **hyperSpec** {#sec:funct-prov-phy}

Table~\@ref(tab:functions)~(p.~\pageref{tab:functions}) in the appendix gives an overview of the
functions implemented by package **hyperSpec**.

# Obtaining Basic Information about package **hyperSpec** Objects
As \mFun{print, show, summary} usual, the `print()`{.r} and `show()`{.r} methods display
information about the object, and `summary()`{.r} yields some additional details about the data
...
```
%
The\mFun{nrow, ncol, nwl, dim} data set `chondro`{.r} consists of `r nrow (chondro)` spectra
with `r nwl (chondro)` data points each, and `r ncol (chondro)` data columns: two for the
spatial information, one factor with the results of a cluster analysis plus `\$spc`{.r}.  These
...
The names of the columns in `@data`{.r} are accessed by\mFun{colnames, rownames, dimnames, wl}

```{r names}
colnames (chondro)
```

Likewise, `rownames()`{.r} returns the names assigned to the spectra, and `dimnames()`{.r} yields
a list of these three vectors (including also the column names of `\$spc`{.r}). The column names
of the spectra matrix contain the wavelengths as character, while `wl()`{.r} (see
...
`colnames<-()`{.r} and `rownames<-()`{.r}, respectively. `colnames<-()`{.r} renames the
labels as well.

# Creating a `hyperSpec`{.r Object, Data Import and Export} {#sec:create}
package **hyperSpec** comes with filters for a variety of file formats. These are discussed in detail in a separate
vignette accessible via `vignette ("fileio")`.

\subsection{Creating a `hyperSpec`{.r} Object from Spectra Matrix
...
-  [{`label`{.r}}] a list with the proper labels. Do not forget the wavelength axis label in
  `\$.wavelength`{.r} and the spectral intensity axis label in `\$spc`{.r}.
...
If package **mvtnorm** is available, multivariate normally distributed spectra can be generated from
mean and covariance matrix using `rmmvnorm()`{.r}\mFun{rmmvnorm} (fig.~\@ref(fig:sim:spc)). Note
that the package **hyperSpec** function's name has an additional" it already takes care of multiple groups.
Mean spectra and pooled covariance matrix can be calculated using
`pooled.cov()`{.r}\mFun{pooled.cov}:
...
```{r simspc, fig.cap=CAPTION, width=10, height=5, include=FALSE}
...
```{r simlda, fig.cap=CAPTION, pnr  width=5, height=5, include=FALSE}
...
If individual covariance matrices should be used for each group, `sigma`{.r} should be an array
with the 3rd dimension corresponding to the group.

# Access to the data {#sec:access-parts}
The main functions to retrieve the data of a `hyperSpec`{.r} object are `[]()`{.r}\mFun{`[]()`{.r},
  `[[]]()`{.r}} and `[[]]()`{.r}.

The difference between these functions is that `[]()`{.r} returns a `hyperSpec`{.r} object, whereas the
result of `[[]]()`{.r} is a `data.frame`{.r} if extra data columns were selected or otherwise the
spectra matrix. Single extra data columns may be retrieved by `\$()`{.r}\mFun{`\$()`{.r}}.

In order to change data, use `[]<-()`{.r}, `[[]]<-()`{.r}, and
`\$<-()`{.r}\mFun{`[]<-()`{.r}, `[[]]<-()`{.r}, `\$<-()`{.r}}
...
`hyperSpec`{.r} comes with three abbreviation functions for easy access to the data:
<!-- `x`{.r --> `[[`i`{.r()`{.r}, , `l`{.r}]] <-} }
-  [`x`{.r} `[[]]()`{.r}] returns the spectra matrix (`x\$spc`{.r}).
-  [`x`{.r} `[[`i`{.r()`{.r}, , `l`{.r}]]}] the cut spectra matrix is returned if
  wavelengths are specified in `l`{.r}.
-  [`x`{.r} `[[`i`{.r()`{.r}, `j`{.r}, `l`{.r}]]}] If data columns are
  selected (second index), the result is a `data.frame`{.r}.
-  [`x`{.r} `[[`i`{.r()`{.r}, , `l`{.r}]] <-}] Also, parts of the spectra matrix
  can be set (only indices for spectra and wavelength are allowed for this function).
-  [`x`{.r} `[`i`{.r()`{.r}, `j`{.r}] <-}] sets parts of `x@data`{.r}.
-  [`x`{.r} `\$.()`{.r}] returns the complete `data.frame`{.r} `x@data`{.r}, with the
  spectra in column `\$spc`{.r}.
-  [`x`{.r} `\$..()`{.r}] returns the extra data (`x@data`{.r} without `x\$spc`{.r}).
-  [`x`{.r} `\$.. <-()`{.r}] sets the extra data (`x@data`{.r} without
  `x\$spc`{.r}). The columns must match exactly in this case.

## Selecting and Deleting Spectra {#sec:select-delete-spectra}

The extraction function `[]()`{.r} takes the spectra as first argument (For detailed help: see
`? `[``). It may be a vector giving the indices of the spectra to extract (select), a vector
with negative indices indicating which spectra should be deleted, or a logical. Note that a matrix
given to `[]()`{.r} will be treated as a vector.
...
sample (chondro, 3)
```
If appropriate indices into the spectra are needed instead, use `isample()`{.r}\mFun{`isample()`{.r}}:
...
### Sequences {#sec:seq}
Sequences of every n^th^ spectrum or the like can be retrieved with `seq()`{.r}\mFun{`seq()`{.r}}:
...
The second argument of the extraction functions `[]()`{.r} and `[[]]()`{.r} specifies the
(extra) data columns. They can be given like any column specification for a `data.frame`{.r}, i.e.,
...
To select one column, the `\$()`{.r} operator is more convenient\mFun{`\$()`{.r}}:

```{r data2}
flu$c
```
`hyperSpec`{.r} supports command line completion for the `\$`{.r} operator.

The extra data may also be set this way\mFun{`\$<-()`{.r}}:
...
## More on the `[[]]()`{.r}and `[[]]<-()`{.r Operators: Accessing Single Elements of the Spectra Matrix} {#sec:square-brack-replace}
`[[]]()`{.r} works mostly analogous to []. In addition, however, these two functions also accept
index matrices of size $n × 2$. In this case, a vector of values from the spectra matrix is
returned.
...
`[[]]<-()`{.r} also accepts index matrices of size $n × 2$.
...
Spectra\mFun{`wl2i()`{.r} `i2wl()`{.r}} in package **hyperSpec** have always discretized wavelength axes, they
are stored in a matrix with each column corresponding to one wavelength. package **hyperSpec** provides two functions
to convert the respective column indices into wavelengths and vice versa: `i2wl()`{.r} and
`wl2i()`{.r}.
...
using a *formula*.  The basic syntax is *start~*$\sim$~*end.* This yields a vector
*index of start *`:`{.r}* index of end.*

The result of the formula conversion differs from the numeric vector conversion in three ways:

-  The colon operator for constructing vectors accepts only integer numbers, the tilde (for
  formulas) does not have this restriction.
-  If the vector does not take into account the spectral resolution, one may get only every
  $n$^th^ point or repetitions of the same index:
...
*start* and *end* may contain the special variables `min`{.r} and `max`{.r} that
...
Often, specifications like *wavelength \textpm $n$\ data points* are needed. They can be given
...
and vectors^[Formulas are combined to a list by `c()`{.r].}:

```{r wl2i.list}
wl2i (flu, c (min ~ 406.5, max - 2i ~ max))
```

This mechanism also works for the wavelength arguments of `[]()`{.r}, `[[]]()`{.r}, and
`plotspc()`{.r}.

### Selecting Wavelength Ranges
Wavelength ranges can easily be selected using `[]()`{.r}'s third argument:
...
spectra matrix, use `wl.index = TRUE`{.r}:
...
2800 cm^-1^ of the paracetamol spectrum one can thus use:
...
wavelength axis is optionally cut in order to save space. For details see the"lottin"vignette).

### Changing the Wavelength Axis {#sec:wavel-axis-conv}
\index{wavelength!conversion} Sometimes wavelength axes need to be transformed, e.g., converting from
wavelengths to frequencies. In this case, retrieve the wavelength axis vector with
`wl()`{.r}\mFun{`wl()`{.r}, `wl<-()`{.r}}, convert each value of the resulting vector and
assign the result with `wl<-()`{.r}. Also the label of the wavelength axis may need to be
adjusted.

As an example, convert the wavelength axis of `laser`{.r} to frequencies. As the wavelengths are
...
If the wavelength axis of an object needs reordering (e.g., after `collapse()`{.r}), `orderwl()`{.r}\mFun{`orderwl()`{.r}} can be used:
...
\mFun{`as.data.frame()`{.r}}`as.data.frame()`{.r} extracts the `@data`{.r} slot as a `data.frame`{.r}:
...
`as.data.frame()`{.r} and the abbreviations \mFun{`\$.()`{.r},
  `\$..()`{.r}}`\$.()`{.r} and `\$..()`{.r} retrieve the usual wide format `data.frame`{.r}{}s:

```{r}
flu$.
flu$..
```
If another subset of colums needs to be extracted, use \mFun{`[[]]()`{.r}}`[[]]()`{.r}:
...
The transpose of a wide format `data.frame`{.r} can be obtained by
\mFun{`as.t.df()`{.r}}`as.t.df()`{.r}. For further examples, see the discussion of
package **ggplot2** in `vignette ("plotting")`{.r}.

```{r}
as.t.df (apply (flu, 2, mean_pm_sd))
```

Some functions need the data being an *unstacked* or *long-format*
`data.frame`{.r}. \mFun{`as.long.df()`{.r}}`as.long.df()`{.r} is the appropriate conversion function.
...
\mFun{`as.matrix()`{.r}, `[[]]()`{.r}} The spectra matrix is extracted by
`as.matrix()`{.r}, the convenient abbreviation is `[[]]()`{.r}:

```{r}
flu [[]]
class (flu [[]])
```
`[[]]()`{.r} takes the same arguments as `[]()`{.r}, and can be used to extract a matrix
...
`hyperspec`{.r} \mFun{`cbind()`{.r} `rbind()`{.r}} Objects can be bound together, either
by columns (`cbind()`{.r}) to append a new spectral range or by row (`rbind()`{.r}) to append
new spectra:
...
There is also a more general function, `bind()`{.r}, taking the direction
(`"r"`{.r} or `"c"`{.r}) as first
...
For binding row-wise (`rbind()`{.r}), `collapse()`{.r}\mFun{`collapse()`{.r}} is
more flexible but also faster.

## Binding Objects that do not Share the Same Extra Data and/or Wavelength Axis {#sec:collapse}

`collapse()`{.r} \mFun{`collapse()`{.r}} combines objects that should be bound together by
...
a particular column or wavelength, its value in the resulting object is `NA`{.r}.

The `barbiturates`{.r} data is a list of `r length (barbiturates)` `hyperSpec`{.r} objects, each
containing one mass spectrum. The spectra have between `r (min (sapply (barbiturates, nwl)))` and
`r (max (sapply (barbiturates, nwl)))` data points each.
...
`merge()`{.r} \mFun{`merge()`{.r}} adds a new spectral range (like `cbind()`{.r}), but
works also if spectra are missing in one of the objects. The arguments `by`{.r}, `by.x`{.r},
and `by.y`{.r} specify which columns should be used to decide which spectra are the same.  The
arguments `all`{.r}, `all.x`{.r}, and `all.y`{.r} determine whether spectra should be
kept for the result set if they appear in only one of the objects.  For details, see also the help on
the base function package **merge**.

As an example, let's construct a version of the `chondro`{.r} data like being taken as two maps
...
```
%
By default, the result consists of only those spectra, where *both* spectral ranges were
available.  To keep all spectra replacing missing parts by `NA`{.r} (see fig.~\@ref(fig:merge)):
...
```{r missing, fig.cap=CAPTION, width=10, height=4.25, echo=FALSE}
...
\caption[]{ {#fig:merge}
  ...
  \subref{fig-merge-mat} The missing parts of the spectra are filled with `NA`{.r}.}
\end{figure}
...
be done by using `approx()`{.r} using `method = "constant"`{.r}.  For duplicated wavelengths,
...
This information can be `merge()`{.r}d into the extra data of `flu`{.r} by:
...
The usual rules for `merge()`{.r} apply. E. g., if to preserver all spectra of flu, use `all.x = TRUE`{.r}:
...
Two `hyperSpec`{.r} objects can be matrix multiplied by `\%*\%()`{.r}\mFun{`\%*\%()`{.r}}. For an
example, see the principal component analysis below (section~\@ref(sec:pca) on page
\pageref{sec:pca}).

## Decomposition {#sec:decomposition}
Matrix decompositions are common operations during chemometric data analysis. The results, e.g., of a
...
Both types of result objects can be"e-importe"into `hyperSpec`{.r} objects with function
`decomposition()`{.r}\mFun{`decomposition()`{.r}}. A scores-like object retains all
per-spectrum information (i.e., the extra data) while the spectra matrix and wavelength vector are
replaced.  A loadings-like object retains the wavelength information, while extra data is deleted
(set to `NA`{.r}) unless the value is constant for all spectra.
...
## Cutting the Spectral Range\mFun{`[]()`{.r} `[[]]()`{.r}}
The extraction functions `[]()`{.r} and `[[]]()`{.r} can be used to cut the spectra: Their
third argument takes wavelength specifications as discussed above and also logicals (i.e. vectors
specifying with `TRUE`{.r}/`FALSE`{.r} for each column of `\$spc`{.r}
whether it should be included or not.

`[]()`{.r} returns a `hyperSpec`{.r} object, `[[]]()`{.r} the spectra matrix `\$spc`{.r}
(or the `data.frame`{.r} `@data`{.r} if in addition data columns were specified) only.
...
\begin{enumerate}
-  The wavelength axis can be shifted, while the intensities stay unaffected.
-  the spectra are interpolated onto a new wavelength axis, while the nominal wavelengths stay.
\end{enumerate}
...
```{r shift-wl, fig.cap=CAPTION, echo=FALSE, include=FALSE}
...
In that case, interpolation is needed.  R offers many possibilities to interpolate (e.g.,
`approx()`{.r} for constant / linear approximation, `spline()`{.r} for spline interpolation,
`loess()`{.r} can be used to obtain smoothed approximations, etc.). The appropriate interpolation
strategy will depend on the spectra, and package **hyperSpec** therefore leaves it up to the user to select a sensible
...
```{r shift-interp, fig.cap=CAPTION, include=FALSE, echo=FALSE, results='hide'}
...
```{r shift-untsch, fig.cap=CAPTION, include=FALSE, echo=FALSE, results='hide'}
...
If different spectra need to be offset by different shift, use a loop^[`sweep()`{.r]
  ...
  is much faster than row-wise extracting and changing the spectra by `[[()`{.r} and
  `[[<-()`{.r}.}
...
\begin{figure}[t]
  \subfloat[ {#fig:shift:wl} `wl<-()`{.r}]{\includegraphics[width=.33\linewidth]{hyperspec-fig-shift-wl}}
  \subfloat[ {#fig:shift:interp} interpolation]{\includegraphics[width=.33\linewidth]{hyperspec-fig-shift-interp}}
  \subfloat[ {#fig:shift:untsch} ]{\includegraphics[width=.33\linewidth]{hyperspec-fig-shift-untsch}}
  \caption[]{Shifting the Spectra along the Wavelength Axis. \subref{fig:shift:wl} Changing the
    ...
    of the phenylalanine band: shifting by `wl<-()`{.r} (red) does not affect the intensities,
...
For the `chondro`{.r} data, let's use the maximum of the phenylalanine band between 990 and
$cm^{-1}${1020}. As just the very maximum is too coarse, we'll use the maximum of a square polynomial
...
for (i in 1 : nrow (chondro))
  shift2 [i] <- unlist (optimize (targetfn, interval = c (-5, 5), wl = chondro```
wavelength,
...
```{r shift-fit, fig.cap=CAPTION, echo=FALSE}
...
1. `hyperSpec`{.r}'s comparison operators return a logical matrix of the size of the spectra that is suitable
for later indexing:
...
```{r filter, fig.cap=CAPTION, width=10, height=5, include=FALSE}
...
Depending on the type of analysis, one may wants to replace the `NA`{.r}s by interpolating the
neighbour values. So far, package **hyperSpec** provides three functions that can interpolate the `NA`{.r}s:
\mFun{`spc.NA.approx()`{.r}, `spc.loess()`{.r}, `spc.bin()`{.r}}:
`spc.NA.approx()`{.r}, `spc.loess()`{.r}, and `spc.bin()`{.r} with \Rcode{na.rm =
...
```{r bad, fig.cap=CAPTION, width=10, height=5, include=FALSE, echo=FALSE, results='hide'}
...
\mFun{`spc.bin()`{.r} `spc.loess()`{.r}} Spectra acquired by grating instruments are
frequently interpolated onto a new wavelength axis, e.g., because the unequal data point spacing should
...
`spc.bin()`{.r} bins the spectral axis by averaging every `by`{.r} data points.
...
```{r fig-loess, fig.cap=CAPTION, width=10, height=5, include=FALSE}
...
```{r fig-loess-kl, fig.cap=CAPTION, width=5, height=5, include=FALSE, echo=FALSE}
...
\caption{ {#fig-spcloess}Smoothing interpolation by `spc.loess()`{.r} with new data point
  spacing of 2 cm^-1^ (red) and `spc.bin()`{.r} (blue). The magnification on the
  right shows how interpolation may cause a loss in signal height.}
\end{figure}

`spc.loess()`{.r} applies R's `loess()`{.r} function for spectral interpolation. Figure
\@ref(fig-spcloess) shows the result of interpolating from 300 to 1800 and 2850 to 3150
cm^-1^ with 2 cm^-1^ data point distance. This corresponds to a
spectral resolution of about 4 cm^-1^, and the decrease in spectral resolution can be
...
`r nwl (paracetamol)` data points with unequal data point spacing (between
`r signif (min (diff (wl (paracetamol))), 2)` and
`r signif (max (diff (wl (paracetamol))), 2)` cm^-1^).  The interpolated spectrum
has `r nwl (p)` data points.

## Background Correction

\mFun{`sweep()`{.r}} To subtract a background spectrum of each of the spectra in an object,
use `sweep (spectra, 2, background.spectrum, "-")`{.r}.

## Offset Correction

\mFun{`apply()`{.r} `sweep()`{.r}} Calculate the offsets and sweep them off the spectra:

```{r ofs}
offsets <- apply (chondro, 1, min)
chondro.offset.corrected <- sweep (chondro, 1, offsets, "-")
```
If the offset is calculated by a function, as here with the `min()`{.r}, `hyperSpec`{.r}'s
`sweep()`{.r} method offers a shortcut: `sweep()`{.r}'s `STATS`{.r} argument may be the
...
\mFun{`spc.fit.poly()`{.r} `spc.fit.poly.below()`{.r}}`spc.fit.poly()`{.r} fits a
...
`spc.fit.poly.below()`{.r} tries to find appropriate support
points for the baseline iteratively.

Both functions return a `hyperSpec`{.r} object containing the
...
For details, see `vignette (baselinebelow)`{.r}.

Package package **baseline** `r cite.pkg("baseline")` offers many more functions for baseline
correction. The `baseline()`{.r} function works on the spectra matrix, which is extracted by
`[[]]()`{.r}. The result is a `baseline`{.r} object, but can easily be re-imported into the
`hyperSpec`{.r} object:
...
```
Fig.~\@ref(fig:bl-baseline) shows the result for the first spectrum of `chondro`{.r}.
\begin{figure}[tb]
...
```{r fig.cap=CAPTION, include=TRUE, width=10, height=5, echo=FALSE, results='hide'}
...
\caption{Baseline correction using the package **baseline** package: the first spectrum of
  `chondro`{.r} with baseline (left) and after baseline correction (right) with method
 "odpolyfi"}
 {#fig:bl-baseline}
\end{figure}
...
\mFun{`sweep()`{.r}} For each of the wavelengths the same correction needs to be applied to all
spectra.
\begin{enumerate}
-  There might be wavelength dependent offsets (background or dark spectra).
  They are subtracted:

  `sweep (spectra, 2, offset.spectrum, "-")`{.r}
-  A multiplicative dependency such as a CCD's photon efficiency: 

  `sweep (spectra, 2, photon.efficiency, "/")`{.r}
\end{enumerate}

### Spectra Dependent Correction

\mFun{`sweep()`{.r}} If the correction depends on the spectra (e. g. due to inhomogeneous
illumination while collecting imaging data, differing optical path length, etc.), the
`MARGIN`{.r}of the `sweep()`{.r} function needs to be 1 or `SPC`{.r}:
\begin{enumerate}
-  Pixel dependent offsets are subtracted:

  `sweep (spectra, SPC, pixel.offsets, "-")`{.r}
-  A multiplicative dependency: 

  `sweep (spectra, SPC, illumination.factors, "{*`{.r}")}
\end{enumerate}

## Normalization
\mFun{`apply()`{.r} `sweep()`{.r}} Again, `sweep()`{.r} is the function of
...
-  Again, sweep the factor off the spectra:
  ...
  `normalized <- sweep (spectra, 1, factors, "{*`{.r}")}
\end{enumerate}
...
\mFun{`scale()`{.r}} Centering means that the mean spectrum is subtracted from each of the
...
fact that for wavelengths with low signal the noise level will"xplod"by variance
scaling. Scaling usually makes sense only for centered data.

Both tasks are carried out by the same method in \R, `scale()`{.r}, which will by default both
...
```{r centre-flu, fig.cap=CAPTION, width=10, height=5}

flu.centered <- scale (flu, scale = FALSE)
plot (flu.centered)
```

On the other hand, the `chondro`{.r} data set consists of Raman spectra, so the spectroscopic
interpretation of centering is getting rid of the the average chemical composition of the
sample. But: what is the meaning of the"verage spectru"of an inhomogeneous sample? In this case
...
order to avoid that, using e.g., the 5^th^ percentile spectrum is more suitable:
...
```{r perc, fig.cap=CAPTION, width=10, height=5, include=TRUE}
...
\mFun{`pls::msc()`{.r}}MSC can be done using `msc()`{.r} from package
package **pls**\citep{Wehrens2007}.  It operates on the spectra matrix:
...
## Spectral Arithmetic \mFun{`+ - {*()`{.r} / \textasciicircum{} log log10}}

Basic mathematical functions are defined for `hyperSpec`{.r} objects.
You may convert spectra:

`absorbance.spectra = - log10 (transmission.spectra)`{.r}

In this case, do not forget to adapt the label:\mFun{`labels()`{.r}}

```{r label, eval=FALSE}
labels (absorbance.spectra)$spc <- "A"
```

Be careful: R's `log()`{.r} function calculates the natural
logarithm if no base is given.

The basic arithmetic operators work element-wise in R. Thus they
all need either a scalar, or a matrix (or `hyperSpec`{.r} object)
of the correct size.

Matrix multiplication is done by `\%{*()`{.r}\%} \mFun{`\%{*()`{.r}\%}}, again each of the
operands may be a matrix or a `hyperSpec`{.r} object, and must have the correct dimensions.

# Data Analysis 
\subsection{Data Analysis Methods using a `data.frame`{.r} 

e. g. Principal Component Analysis with `prcomp()`{.r}} {#sec:pca}
\mFun{`\$.()`{.r}} The `\$.`{.r} notation is handy, if a data analysis function expects
a `data.frame`{.r}. The column names can then be used in the formula:

```{r pca}
pca <- prcomp (~ spc, data = chondro$., center = FALSE)
```

Many modeling functions call `as.data.frame`{.r} on their `data`{.r} argument. In that case, the conversion is done automatically:

```{r pca-auto}
pca <- prcomp (~ spc, data = chondro, center = FALSE)
```

Results of such a \mFun{`decomposition()`{.r}} decomposition can be put again into
`hyperSpec`{.r} objects. This allows to plot e.g., the loading like spectra, or score maps, see
figure~\@ref(fig:pca).
...
`retain.columns`{.r} triggers whether columns that contain different values should be dropped. If
it is set to `TRUE`{.r}, the columns are retained, but contain `NA`{.r}s:
...
```{r pca-load, fig.cap=CAPTION, width=8, height=4, echo=FALSE}

plot (loadings [1:3], stacked = TRUE)
```
}
\subfloat[ {#fig-pca-score}]{
...
```{r pca-score, fig.cap=CAPTION, width=8, height=4, echo=FALSE}

plotmap (scores [,,3], col.regions = div.palette (20))
```
}
\caption[]{ {#fig:pca}
  \subref{fig-pca-load} The first three loadings: `plot (loadings [1 : 3], stacked = TRUE)`{.r}.
  \subref{fig-pca-score} The third score map: `plotmap (scores [, , 3])`{.r}.}
\end{figure}
...
If we decide that only the first 10 components actually carry spectroscopic information, we can reconstruct spectra with better signal to noise ratio\mFun{`\%*\%()`{.r}}:

```{r pca-smooth}
smoothed <- scores [,, 1:10] %*% loadings [1:10]
```
Keep in mind, though, that we cannot be sure how much *useful* information was discarded with the higher components. This kind of noise reduction may influence further modeling of the data. Mathematically speaking, the rank of the new
`r nrow (chondro)` $×$ `r nwl (chondro)` spectra matrix is only 10.

## Data Analysis using long-format `data.frame`{.r}
 e.g., plotting with ggplot2 {#sec:as.long.df}
Some functions need the data being an *unstacked* or *long-format* `data.frame`{.r}. \mFun{`as.long.df()`{.r}}`as.long.df()`{.r} is the appropriate conversion function.
...
e.g., Hierarchical Cluster Analysis}
 {#HCA}\mFun{`[{()`{.r}[{}]{}]}}
Some functions expect their input data in a matrix, so either `as.matrix (object)`{.r} or the
abbreviation `object [[]]`{.r} can be used:
...
```{r dend, fig.cap=CAPTION, width=5, height=5, include=FALSE}
...
```{r clustmap, fig.cap=CAPTION, width=7.5, height=5, include=FALSE, echo=FALSE}

plotmap (chondro, clusters ~ x * y, col.regions = cluster.cols)
```
\begin{figure}
\subfloat[ {#fig:clust:dend}]{\includegraphics[width=0.33\textwidth]{hyperspec-fig-dend}}
\subfloat[ {#fig:clust:map}]{\includegraphics[width=0.33\textwidth]{hyperspec-fig-clustmap}}
\subfloat[ {#fig:clust:spc}]{\includegraphics[width=0.33\textwidth]{hyperspec-fig-clustmean}}
...
`aggregate()`{.r}\mFun{`aggregate()`{.r}} applies the function given in `FUN`{.r}
to each of the groups of spectra specified in `by`{.r}.
...
```{r clustmean, fig.cap=CAPTION, width=7.5, height=5, include=FALSE}
...
For subsections of `hyperSpec`{.r} objects that do not contain all levels of a factor column, \mFun{`droplevels()`{.r}}`droplevels()`{.r} drops the"npopulate"levels: 
...
\mFun{`split()`{.r}}A `hyperSpec`{.r} object may also be split into a list of `hyperSpec`{.r}
objects:
...
Splitting can be reversed by `rbind()`{.r}\mFun{`rbind()`{.r}} (see section~\@ref(sec:bind),
page~\pageref{sec:bind}).  Another, similar way to combine a number of `hyperSpec`{.r} objects with different
wavelength axes or extra data columns is `collapse()`{.r}\mFun{`collapse()`{.r}} (see
...
While most of package **hyperSpec**'s functions work at a decent speed for interactive sessions (of course depending
...
Calculations that involve a lot of subsetting (i.e., extracting or changing the spectra matrix or extra
data) can be sped up considerably if the required parts of the `hyperSpec`{.r} object are extracted
beforehand. This is somewhat similar to model fitting in R in general: many model fitting functions
in \R are much faster if the formula interface is avoided and the appropriate `data.frame`{.r}{}s or matrices are
...
\begin{description}
- **package **matrixStats**** implements fast functions to calculate summary statistics for each row
  or each column of a matrix. This functionality can be enabled for package **hyperSpec** by installing package
  package **hyperSpec.matrixStats** which is available in package **hyperSpec**'s development repository at
  <http://hyperSpec.r-forge.r-project.org/>
\end{description}

#### Compiled code. R provides interfaces to Fortran and C code,
see the manual"riting R Extension" package **Rcpp**`r cite.pkg("Rcpp")` allows to
conveniently integrate C++ code. package **inline**`r cite.pkg("inline")` adds another layer of
...
package **compiler**`r cite.pkg("compiler")`.

#### Memory use. In general, it is recommended not to work with variables that are more than
approximately a third of the available RAM in size. Particularly the import of raw spectroscopic data
can consume large amounts of memory. At certain points, package **hyperSpec** provides switches that allow working
with data sets that are actually close to this memory limit.

\mFun{`new ("hyperSpec")`{.r}, `read.ENVI*()`{.r},
`read.txt.Renishaw()`{.r}}\index{options!gc}
The initialization method `new ("hyperSpec", ...)`{.r} takes particular care to avoid
unneccessary copies of the spectra matrix. In addition, frequent calls to `gc ()`{.r} can be
requested by `hy.setOption (gc = TRUE)`{.r}. The same behaviour is triggered in
`read.ENVI()`{.r} and its derivatives (`r texListFun("read.ENVI")`). The memory consumption
of `read.txt.Renishaw()`{.r} can be lowered by importing the data in chunks (argument
`nlines`{.r}).
...
```{r tab-fn, results='tex', echo=FALSE}
...
\n", sep = "")
  ...
  cat (paste (paste ("\`", df[1,], "`", sep = ""), df[2,], sep = " & ", collapse ="
...
#  compactPDF ('$```
', qpdf =" gs_quality = 'screen', gs_extras = '-dDownsampleColorImages=false')
@
%
\end{footnotesize}
