...
make_bib(
  c("hyperSpec", "baseline", "compiler", "Rcpp", "inline", "mvtnorm"),
...
CAPTION = "The structure of the data in a package **hyperSpec** object.  "
```
...
suitable for axis annotations, i.e., they should be either character strings or expressions for"rett"axis annotations (see e.g., figure \@ref(fig-spcloess) on page \pageref{fig-spcloess}).
To get familiar with expressions for axis annotation, see `? plotmath` and `demo (plotmath)`.
...
\mFun{rmmvnorm}
If package **mvtnorm**[@mvtnorm] is available, multivariate normally distributed spectra can be generated from mean and covariance matrix using `rmmvnorm()`{.r} (Fig. \@ref(fig:sim-spc)). 
...
```{r missing, fig.cap=CAPTION, echo=FALSE}
...
## Cutting the Spectral Range\mFun{`[]` `[[]]`}
The extraction functions `[]` and `[[]]` can be used to cut the spectra: Their
...
`[]` returns a `hyperSpec`{.r} object, `[[]]` the spectra matrix `$spc`{.r}
...
```{r shift-wl, fig.cap=CAPTION, echo=FALSE}
plot(chondro[135])
...
```
<!-- , fig.width=10, fig.height=5 -->
```{r bad, fig.cap=CAPTION, results='hide'}
spc[[is.na(spc)]] <- 0
plot(spc)
spc[[spc < 1e-4]] <- NA
...
```{r fig-loess-kl, fig.cap=CAPTION}
...
```
<!-- , fig.width=10, fig.height=5 -->
```{r fig.cap=CAPTION, include=TRUE, echo=FALSE, results='hide'}
baseline <- corrected
...
```{r pca-load, fig.cap=CAPTION, echo=FALSE}
...
\mFun{`aggregate()`{.r}}
Function `aggregate()`{.r} applies the function given in `FUN`{.r}
...
\mFun{`droplevels()`{.r}}
For subsections of `hyperSpec`{.r} objects that do not contain all levels of a factor column, `droplevels()`{.r} drops the"npopulate"levels: 
