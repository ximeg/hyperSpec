...
data sets, i.e., data sets combining spectra with further data on a per-spectrum basis.
...
-  `barbiturates`{.r} \index{data sets!barbiturates}GC-MS spectra with differing wavelength axes as a list of `r length(barbiturates)` `hyperSpec`{.r} objects.
...
- **wavelength**: \index{wavelength} spectral abscissa  frequency, wavenumbers, chemical shift, Raman shift, $\frac{m}{z}$, etc.
- **intensity**:] \index{intensity}spectral ordinate transmission, absorbance, $\frac{e^{-}}{s}$, intensity, \textellipsis
-  **extra data**: \index{extra data}further information/data belonging to each spectrum spatial information (spectral images, maps, or profiles), temporal information (kinetics, time series), concentrations (calibration  series), class membership information, etc.
  Class `hyperSpec`{.r} object may contain arbitrary numbers of extra data columns.

In R, slots of a S4 class are accessed by the \index{`"@`{.r} operator}`@`{.r} operator.
In this vignette, the notation `@xxx`{.r} will thus mean \emph"lot xxx of an objec". 
Likewise, named elements of a `list`{.r} and columns of a `data.frame`{.r} are accessed by the \index{`$`{.r} operator}`$`{.r} operator, and `\$xxx`{.r} will be used for \emph"olumn xx", and as an
...
```{r simlda, fig.cap=CAPTION,  width=5, height=5, include=FALSE}
...
R offers many possibilities to interpolate (e.g.,
`approx()`{.r} for constant / linear approximation, `spline()`{.r} for spline interpolation,
`loess()`{.r} can be used to obtain smoothed approximations, etc.).
...
For the `chondro`{.r} data, let's use the maximum of the phenylalanine band between 990 and $cm^{-1}${1020}.
...
find.max <- function(y, x) {
  ...
  X <- cbind(1, X, X^2)
  coef <- qr.solve(X, Y) - coef[2] / coef[3] / 2 + x[pos[2]]
}
...
<!-- \caption{ -->
<!-- The shifts used to disturb the chondrocyte data (original), and the remaining shift after correction with the two methods discussed here. -->
<!-- } {#fig:fit-shift} -->
<!-- \end{figure} -->
...
`hyperSpec`{.r}'s comparison operators return a logical matrix of the size of the spectra that is suitable for later indexing:
...
CAPTION <- " ??? "
```
```{r fig-loess-2, fig.cap=CAPTION, width=10, height=5, include=FALSE}
...
Figure \@ref(fig-spcloess) shows the result of interpolating from 300 to 1800 and 2850 to 3150 cm^-1^ with 2 cm^-1^ data point distance.
...
<!-- \mFun{`sweep()`{.r}} -->
To subtract a background spectrum of each of the spectra in an object, use `sweep (spectra, 2, background.spectrum, "-")`{.r}.
...
The `baseline()`{.r} function works on the spectra matrix, which is extracted by `[[]]()`{.r}.
...
be immediately subtracted: `spectra - constant`{.r}
...
<!-- \mFun{`pls::msc()`{.r}} -->
MSC can be done using `msc()`{.r} from package package **pls**\citep{Wehrens2007}. 
...
Matrix multiplication is done by `\%{*()`{.r}\%} \mFun{`\%{*()`{.r}\%}}, again each of the operands may be a matrix or a `hyperSpec`{.r} object, and must have the correct dimensions.
...
This allows to plot e.g., the loading like spectra, or score maps, see figure \@ref(fig:pca).
...
```{r pca-score, fig.cap=CAPTION, width=8, height=4}
...
see the manual "Writing R Extensions" package **Rcpp**`r cite_pkg("Rcpp")` allows to
conveniently integrate C++ code. package **inline**`r cite_pkg("inline")` adds another layer of
...
package **compiler**`r cite_pkg("compiler")`.
...
`read.ENVI()`{.r} and its derivatives (`r texListFun("read.ENVI")`). 
...
# Overview of the functions provided by package **hyperSpec** {#tab:functions}
