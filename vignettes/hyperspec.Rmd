...
link-citations: yes
bibliography: man/hyperspec-pkg.bib
biblio-style: plain
...
make_bib(c("hyperSpec", "baseline", "compiler", "Rcpp", "inline"),
  file = "man/hyperspec-pkg.bib"
)
```
...
[R FAQ: Why do lattice/trellis graphics not work?](http://cran.r-project.org/doc/FAQ/R-FAQ.html#Why-do-lattice_002ftrellis-graphics-not-work_003f)) and allow the code in the code chunks of the
...
\index{operators}Operators such as `+`, `-`,`*`, `%%`, etc. are in fact functions
in R. Thus they
can be handed over as arguments to other functions (particularly to the vectorization functions
`*apply()`{.r}, `sweep()`{.r}, etc.). In this case t he name of the function must be quoted:
...
usual, the `print()`{.r} and `show()`{.r} methods display
...
data set `chondro`{.r} consists of `r nrow (chondro)` spectra
...
data column names and rownames of the object may be set by
...
\mFun{`as.t.df()`{.r}}
`as.t.df()`{.r}. For further examples, see the discussion of
...
`spc.NA.approx()`{.r}, `spc.loess()`{.r}, and `spc.bin()`{.r} with `na.rm = TRUE`{.r} (the latter two are discussed below).
...
Spectra acquired by grating instruments are
...
\mFun{`apply()`{.r} 
`sweep()`{.r}} Calculate the offsets and sweep them off the spectra:
...
\mFun{`spc.fit.poly()`{.r} 
`spc.fit.poly.below()`{.r}}`spc.fit.poly()`{.r} fits a
...
1.  Pixel dependent offsets are subtracted: `sweep (spectra, SPC, pixel.offsets, "-")`{.r}
2.  A multiplicative dependency:  `sweep (spectra, SPC, illumination.factors, "{*}")`{.r}
\end{enumerate}
...
\mFun{`scale()`{.r}} 
Centering means that the mean spectrum is subtracted from each of the
...
\mFun{`pls::msc()`{.r}}
MSC can be done using `msc()`{.r} from package
...
## Spectral Arithmetic 
\mFun{`+ - {*()`{.r} / \textasciicircum{} log log10}}
...
\mFun{`split()`{.r}}A `hyperSpec`{.r} object may also be split into a list of `hyperSpec`{.r} objects:
...
R provides interfaces to Fortran and C code,
see the manual "Writing R Extensions" package **Rcpp**`r cite.pkg("Rcpp")` allows to
...
In general, it is recommended not to work with variables that are more than
