...
\title{Unstable Laser Emission Vignette for the Data Set `laser` of the R package package **hyperSpec** }
...
`hyperSpec`{.r} objects  can easily be drawn with package **rgl**`r cite.pkg ("rgl")`:

```{r libraryrgl}
require(rgl)
```
package **rgl**'s `persp3d()`{.r} plots a surface in 3d defined by points in x, y, and z. Handing over the appropriate data columns of the `hyperSpec`{.r} object is easy (fig.~\@ref(fig:laser3d)):
...
```{r makebib, echo=FALSE, results='hide'}
